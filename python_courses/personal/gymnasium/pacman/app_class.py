...
class App:
    def __init__(self):
        ...
        self.running = True
        self.state = 'start'
        self.cell_width = maze_width // coloumns
    ...
    def load(self):
        self.background = pygame.image.load('layout_maze.png')
        ...
        # stored as  a vector
        with open("layout_walls.txt", 'r') as file:
    ...
    def reset(self):
        ...
        self.player.pos = self.player.get_pos()
        self.player.direction *= 0
        for enemy in self.enemies:
            enemy.grid_pos = vec(enemy.starting_pos)
            enemy.pos = enemy.get_pos()
            enemy.direction *= 0
    ...
    def start_draw(self):
        ...
        self.draw_text('PUSH SPACE BAR', self.screen, [
                       width // 2, height // 2 - 50], text_size, (170, 132, 58), font, centered=True)
        self.draw_text('1 PLAYER ONLY', self.screen, [
                       width // 2, height // 2 + 50], text_size, (44, 167, 198), font, centered=True)
        self.draw_text('HIGH SCORE', self.screen, [
                       4, 0], text_size, (255, 255, 255), font)
    ...
    def playing_draw(self):
        ...
        self.draw_text('CURRENT SCORE: {}'.format(self.player.current_score),
                       self.screen, [60, 0], 18, white, font)
        self.draw_text('HIGH SCORE: 0', self.screen, [
                       width // 2 + 60, 0], 18, white, font)
        self.player.draw()
        for enemy in self.enemies:
            enemy.draw()
        pygame.display.update()
    ...
    def draw_coins(self):
        for coin in self.coins:
            pygame.draw.circle(self.screen, (124, 123, 7), (int(coin.x * self.cell_width) + self.cell_width //
                               2 + buffer // 2, int(coin.y * self.cell_height) + self.cell_height // 2 + buffer // 2), 5)
