...
## Connection from hardware to website via the server

- How we fetch the data from our database via our server

```javascript
async function fetch_day(days) {
    var date = new Date()
    date.setDate(date.getDate() - days)
    var dateString = date.toISOString().substring(0, 16).replace('T', ' ')
